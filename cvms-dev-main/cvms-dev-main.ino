...
const char* devname = "dev_1"; // Device name
...
void sendSensorDataToServer(String uid) {
  ...
  String url = "http://192.168.7.228/cvms/dev-api/dev_1.php";
  String postData = "sensor1=" + uid + "&devname=" + devname;

  http.begin(client, url); // Use HTTP POST request
  ...
  if (httpResponseCode > 0) {
    ...
    handleServerResponse(response); // Handle the server response
  } else {
...
void handleServerResponse(String response) {
  ...
  if (response == "YES") {
    lcd.setCursor(5, 1); 
    lcd.print("ENTERED");
    tone(BUZZER_PIN, 1000, 300); // Single beep for ENTERED
  } else if (response == "EXIT") {
    lcd.setCursor(5, 1); 
    lcd.print("EXITED");
    tone(BUZZER_PIN, 1000, 300); // First beep
    delay(500);
    tone(BUZZER_PIN, 1000, 300); // Second beep
  } else {
    lcd.setCursor(1, 1); 
...
void handleHTTPError(int httpResponseCode) {
  ...
  tone(BUZZER_PIN, 1000, 300); // Error beep
  delay(500);
